...
class EmojiTextFieldDelegate : NSObject, UITextFieldDelegate {
    ...
    func textField(textField: UITextField, shouldChangeCharactersInRange range: NSRange, replacementString string: String) -> Bool {
        ...
        // For each dictionary entry in translations, pull out a string to search for an emoji to replace it with
        for (emojiString, emoji) in translations {

            // Search for all occurrences of key (e.g. "dog"), and replace with emoji (e.g. 🐶)
            do {
        ...
        // If we have replaced an emoji, then we directly edit the text field.  Otherwise we allow the proposed edit
        if replacedAnEmoji {
